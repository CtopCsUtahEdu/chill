...
// first, remove / replace loop variables 
  ...
  debug_fprintf(stderr, "ordered_cudaized_stmts[CS].second %d\n", ordered_cudaized_stmts[CS].second);
  ...
  for (int i = 0; i < arrayVars.size(); i++) { 
    ...
    if (param->numdimensions > 1) { 
      ...
      for (int i=1; i<param->numdimensions; i++) { 
        ...
        kcall->addArg( CE );
        debug_fprintf("CastExpr is "); CE->print(0,stderr); debug_fprintf(stderr, "\n\n"); 
      }
      //int l = strlen(line);
      //char *ptr = line + l;
      //sprintf(ptr, ")");

...
indices ={}
 for i=1, #indices_to_flatten do
   ...
   if(i==1) then
     count = find_cur_level(stmt_num,indices_to_flatten[i])
     temp = count
   else
     temp = find_cur_level(stmt_num,indices_to_flatten[i])    
   

   if (temp == (count + 1)) then
     indices[i] = temp
   elseif(temp > count + 1) then
      print("temp is " .. temp.. " and  count is " .. count)

      for j = count + 1, temp-1 do
        if (# cur[j] > 0) then
          error("indices not continuous")
        end
      end
   else 

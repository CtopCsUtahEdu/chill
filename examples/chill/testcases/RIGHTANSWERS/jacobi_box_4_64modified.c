...
void smooth_box_4_64(domain_type *domain, int level, int box_id, int phi_id, int rhs_id, double x, double y, int sweep) {
  ...
  for (t2 = -6; t2 <= 70; t2 += 1) {
    ...
    for (t4 = ((-s - 1) % 2 > t2 - 66 + (-s - 1 - (t2 - 66)) % 2 ? (-s - 1) % 2 : t2 - 66 + (-s - 1 - (t2 - 66)) % 2); t4 <= (3 < (t2 + 3) / 3 ? 3 : (t2 + 3) / 3); t4 += 2) 
      for (t6 = t4 - 3; t6 <= -t4 + 66; t6 += 1) 
        for (t8 = -3; t8 <= 66; t8 += 1) 
          _in[-(2 * t4) + t2][t6][t8 + 1] = -128 / 30 * _out[-(2 * t4) + t2][t6][t8] + 14 / 30 * (_out[-(2 * t4) + t2 - 1][t6][t8] + _out[-(2 * t4) + t2][t6 - 1][t8] + _out[-(2 * t4) + t2][t6 + 1][t8] + _out[-(2 * t4) + t2 + 1][t6][t8] + _out[-(2 * t4) + t2][t6][t8 - 1] + _out[-(2 * t4) + t2][t6][t8 + 1]) + 3 / 30 * (_out[-(2 * t4) + t2 - 1][t6][t8 - 1] + _out[-(2 * t4) + t2][t6 - 1][t8 - 1] + _out[-(2 * t4) + t2][t6 + 1][t8 - 1] + _out[-(2 * t4) + t2 + 1][t6][t8 - 1] + _out[-(2 * t4) + t2 - 1][t6 - 1][t8] + _out[-(2 * t4) + t2 - 1][t6 + 1][t8] + _out[-(2 * t4) + t2 + 1][t6 - 1][t8] + _out[-(2 * t4) + t2 + 1][t6 + 1][t8] + _out[-(2 * t4) + t2 - 1][t6][t8 + 1] + _out[-(2 * t4) + t2][t6 - 1][t8 + 1] + _out[-(2 * t4) + t2][t6 + 1][t8 + 1] + _out[-(2 * t4) + t2 + 1][t6][t8 + 1]) + 1 / 30 * (_out[-(2 * t4) + t2 - 1][t6 - 1][t8 - 1] + _out[-(2 * t4) + t2 - 1][t6 + 1][t8 - 1] + _out[-(2 * t4) + t2 + 1][t6 - 1][t8 - 1] + _out[-(2 * t4) + t2 + 1][t6 + 1][t8 - 1] + _out[-(2 * t4) + t2 - 1][t6 - 1][t8 + 1] + _out[-(2 * t4) + t2 - 1][t6 + 1][t8 + 1] + _out[-(2 * t4) + t2 + 1][t6 - 1][t8 + 1] + _out[-(2 * t4) + t2 + 1][t6 + 1][t8 + 1]);
    for (t4 = ((-s - 1) % 2 > t2 - 66 + (-s - 1 - (t2 - 66)) % 2 ? (-s - 1) % 2 : t2 - 66 + (-s - 1 - (t2 - 66)) % 2); t4 <= (3 < (t2 + 3) / 3 ? 3 : (t2 + 3) / 3); t4 += 2) 
      for (t6 = t4 - 3; t6 <= -t4 + 66; t6 += 1) 
        for (t8 = -3; t8 <= 66; t8 += 1) 

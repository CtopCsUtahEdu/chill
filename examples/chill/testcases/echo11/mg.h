...
//											// +1.0 for neumann boundary conditions in finite volume or difference
  ...
  //ijk2 dim;								// dimensions of the buffer to copy
  struct {int i,j,k;} dim;
  struct {
     int box, i, j, k, pencil, plane;
     double* __restrict__ ptr;
  } read;	// coordinates in the read grid to extract the buffer, 
  struct {
     int box, i, j, k, pencil, plane;
     double* __restrict__ ptr;
  } write;	// coordinates in the read grid to extract the buffer, 

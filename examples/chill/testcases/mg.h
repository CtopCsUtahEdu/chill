...
//											// +1.0 for neumann boundary conditions in finite volume or difference
  ...
  //chill currently dies if the following read and write are on the same line 
  struct {int box, i, j, k, pencil, plane;double * __restrict__ ptr;}read, write;	// this dies 

  //struct {int box, i, j, k, pencil, plane;double * __restrict__ ptr;}read;	// coordinates in the read grid to extract the buffer, 
  //struct {int box, i, j, k, pencil, plane;double * __restrict__ ptr;}write;	// coordinates in the read grid to extract the buffer,
...
typedef struct {
  ...
  int vcycles_performed, Krylov_iterations, CAKrylov_formations_of_G;
...
//int                             buffer_size[MaxLevels][27]; // = MPI buffer size (one per neighbor) in doubles
  ...
  int * vector_ghosts;				// vector of ghost zone depths, one of each level
} domain_type;
...
//------------------------------------------------------------------------------------------------------------------------------
 int create_subdomain(subdomain_type * box, 
                      ...
                      int numGrids, int ghosts, int numLevels, int *vec_ghosts);
...
void     FMGSolve(domain_type * domain, int u_id, int F_id, double a, double b, double desired_mg_norm);
